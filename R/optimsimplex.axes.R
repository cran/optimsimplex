...
optimsimplex.axes <- function(x0=NULL,fun=NULL,len=NULL,data=NULL){

  newobj <- optimsimplex.coords()$newobj
  if (size(x0,1)!=1)
    stop(sprintf('optimsimplex.axes: The x0 vector is expected to be a row matrix, but current shape is %d x %d.',
                 size(x0,1),size(x0,2)),
         call.=FALSE)
  if (!is.null(fun))
  ...
  if (is.null(len)){
    ...
    assert.typereal(var=len,varname='len',ivar=1)
    if (size(len,1)!=1)
      stop(sprintf('optimsimplex.axes: The len vector is expected to be a row matrix, but current shape is %d x %d',
                   size(len,1),size(len,2)),
           call.=FALSE)
  }
  assert.typereal(var=x0,varname='x0',ivar=3)
  n <- length(x0)
  newobj$n <- n
  newobj$nbve <- n+1
  nl <- length(len)
  if (nl==1){
    ...
    if (size(len,2) != size (x0,2))
      stop(sprintf(paste('optimsimplex.axes: The len vector is not consistent with the x0 point. ',
